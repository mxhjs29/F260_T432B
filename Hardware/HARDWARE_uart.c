...
/******************************************************************************
  ...
  * A2连接S.BUS总线，波特率为100K       P3.3  P3.2
  * A0连接ANO光流，波特率为115200       P1.2  P1.3
  * A3连接OpenMV，波特率115200
  * A1连接OpenMV，波特率115200
******************************************************************************/
...
void USART_Init()
{
    ...
    EUSCI_A_CMSIS( UsartGroup[UART_A0].moduleInstance)->IFG &= ~(EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
    EUSCI_A_CMSIS( UsartGroup[UART_A1].moduleInstance)->IFG &= ~(EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
    EUSCI_A_CMSIS( UsartGroup[UART_A2].moduleInstance)->IFG &= ~(EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
    EUSCI_A_CMSIS( UsartGroup[UART_A3].moduleInstance)->IFG &= ~(EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
...
void USART_TX(Usart_t *usart, uint8_t *pTx, uint8_t len)
{
    for (int i = 0; i < Num_USART; i++)
    {
        ...
        {
            EUSCI_A_CMSIS(UsartGroup[i].moduleInstance)->IFG |= UCTXIFG;
...
void EUSCIA0_IRQHandler(void)
{
    ...
    if (status & EUSCI_A_UART_RECEIVE_INTERRUPT_FLAG)
    {
        ...
        if (UsartGroup[UART_A0].RxCnt > MAX_RECEIVE_CNT)
        {
            UsartGroup[UART_A0].RxCnt = MAX_RECEIVE_CNT;
        }
